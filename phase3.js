export function registerPhase3({ app, pool, requireAdmin, requireHigher }) {
  ...
  app.get('/api/admin/oversight', requireAdmin, async (_, res) => {
    const r = await pool.query(`SELECT COUNT(*) FILTER (WHERE c.status <> 'RESOLVED')::int AS open_count,
  ...
  app.get('/api/admin/overdue', requireAdmin, async (_, res) => {
    const r = await pool.query(`SELECT c.complaint_code,c.category,c.urgency,c.status,c.complaint_destination,c.created_at,c.updated_at,
      ROUND(${ageHours}::numeric,1) AS age_hours FROM complaints c
      WHERE c.complaint_destination IN ('ADMIN','ESCALATED') AND ${overdueCase}
  ...
  app.get('/api/higher/overdue', requireHigher, async (_, res) => {
    const r = await pool.query(`SELECT c.complaint_code,c.category,c.urgency,c.status,c.complaint_destination,c.created_at,c.updated_at,
      ROUND(${ageHours}::numeric,1) AS age_hours FROM complaints c
      WHERE c.complaint_destination IN ('HIGHER_AUTHORITY','ESCALATED') AND ${overdueCase}
      ORDER BY CASE c.urgency WHEN 'तातडीची' THEN 1 WHEN 'महत्त्वाची' THEN 2 ELSE 3 END, c.created_at ASC LIMIT 200`);
    res.json(r.rows);
  });

  app.get('/api/admin/complaints/:code/audit', requireAdmin, async (req, res) => {
    const r = await pool.query(`SELECT action,old_status,new_status,old_destination,new_destination,note,created_at
      FROM complaint_audit WHERE complaint_code=$1 ORDER BY created_at DESC LIMIT 100`, [req.params.code.toUpperCase()]);
    res.json(r.rows);
  });
  ...
  ensurePhase3().then(async () => {
    await pool.query(`CREATE OR REPLACE FUNCTION safevoice_audit_complaint() RETURNS trigger AS $$
      BEGIN
    ...
    await pool.query(`DROP TRIGGER IF EXISTS safevoice_complaint_audit ON complaints`);
    await pool.query(`CREATE TRIGGER safevoice_complaint_audit AFTER UPDATE OF status, complaint_destination ON complaints FOR EACH ROW EXECUTE FUNCTION safevoice_audit_complaint()`);
  }).catch(err => console.error('Phase 3 database setup failed:', err.message));
}
